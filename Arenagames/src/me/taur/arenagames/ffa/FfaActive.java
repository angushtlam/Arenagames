			...
			if (room.isGameInProgress()) {
				if (room.getPlayersInRoom() == 1) { // If there is only 1 player left.
					if (room.getRoomType() == RoomType.FFA) {
						FfaRoom ffaroom = (FfaRoom) room;
						ffaroom.gameOverMessage(ffaroom.getWinningPlayer());
						
						if (ffaroom.getPlayers() != null) {
							for (Player p : ffaroom.getPlayers()) {
								if (p != null) {
